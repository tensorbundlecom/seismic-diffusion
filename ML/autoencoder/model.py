...
def get_model(in_channels=3, latent_channels=128, device='cuda'):
    """
...
class Decoder(nn.Module):
    ...
    Mirrors the 3-level encoder: latent (C, H/8, W/8) -> (out_channels, H, W).
    """
    def __init__(self, out_channels=3, latent_channels=4, use_latent_projection=False):
        ...
        if use_latent_projection:
            ...
            self.fc_projection = nn.Linear(latent_channels, self.projection_dim)
            
        # Decoding layers — mirror the 3-level encoder.
        # ConvTranspose2d output sizes are approximate; VariationalAutoencoder.forward
    ...
    def forward(self, x):
        if self.use_latent_projection:
            # x is (batch_size, latent_channels) - 1D vector
            batch_size = x.size(0)
            h = self.fc_projection(x)
            h = h.view(batch_size, 256, self.spatial_dim, self.spatial_dim)
            return self.decoder(h)
        else:
            # x is already spatial (batch_size, latent_channels, H/8, W/8)
...
class VAEEncoder(nn.Module):
    ...
    def __init__(self, in_channels=3, latent_channels=128):
        ...
        # Shared encoding layers — 3 stride-2 convolutions give 8× spatial downsampling.
        # Input (3, 129, 111) → (32, 65, 56) → (64, 33, 28) → (128, 17, 14)
        ...
        self.conv_mu = nn.Conv2d(128, latent_channels, kernel_size=3, padding=1)
        self.conv_logvar = nn.Conv2d(128, latent_channels, kernel_size=3, padding=1)
...
class VariationalAutoencoder(nn.Module):
    ...
    def __init__(self, in_channels=3, latent_channels=128):
        ...
        self.decoder = Decoder(out_channels=in_channels, latent_channels=latent_channels, use_latent_projection=False)
