...
def main():
    parser = argparse.ArgumentParser(description="Visualize WFlow post-training OOD (specs + waveforms).")
    ...
    parser.add_argument(
        "--output_dir",
        default="ML/autoencoder/experiments/LegacyCondFlow/visualizations/post_training_ood_evaluation",
    ...
    wflow_chk = 'ML/autoencoder/experiments/LegacyCondFlow/checkpoints/wflow_external_best.pt'
    ...
    wf_state = torch.load(wflow_chk, map_location=device)
    cfg = wf_state.get('config', {})
    wflow = WFlowCVAE(
        in_channels=3,
        latent_dim=cfg.get('latent_dim', 128),
        num_stations=cfg.get('num_stations', num_stations),
        flow_layers=cfg.get('flow_layers', 8),
        w_dim=cfg.get('w_dim', 64),
    ).to(device)
    wflow.load_state_dict(wf_state['model_state_dict'])
    wflow.eval()
    ...
    with torch.no_grad():
        for i in range(len(dataset)):
            ...
            wav_wflow = reconstruct_signal(wflow_spec, mag_min, mag_max, fs=target_fs)

            min_len = min(len(gt_wav), len(wav_base), len(wav_fc), len(wav_flow), len(wav_wflow))
            ...
            m_wflow = calculate_metrics(gt_wav[:min_len], wav_wflow[:min_len], orig_spec, wflow_spec, fs=target_fs)
            ...
            axes[1, 1].imshow(wflow_spec, aspect='auto', origin='lower', cmap='viridis')
            axes[1, 1].set_title(f'WFlow (SSIM: {m_wflow["ssim"]:.3f})')
            axes[1, 2].axis('off')
            plt.tight_layout()
            ...
            fwf, faswf = get_fas(wav_wflow[:min_len], fs=target_fs)
            ...
            plt.loglog(fwf, faswf, 'm', alpha=0.8, label=f'WFlow (LSD: {m_wflow["lsd"]:.2f})', linewidth=0.9)
            ...
            axes[4].plot(time, wav_wflow[:min_len], 'm-', linewidth=0.8, label=f'WFlow (XCorr: {m_wflow["xcorr"]:.3f})')
