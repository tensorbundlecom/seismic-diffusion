...
def load_models(device, num_stations):
    ...
    for p in [baseline_chk, full_cov_chk, flow_chk, wflow_chk]:
    ...
    wflow_state = torch.load(wflow_chk, map_location=device)
    cfg = wflow_state.get('config', {})
    wflow = WFlowCVAE(
        in_channels=3,
    ...
    wflow.load_state_dict(wflow_state['model_state_dict'])
    wflow.eval()

    return baseline, full_cov, flow, wflow


def main():
    parser = argparse.ArgumentParser(description='Evaluate WFlow post-training OOD metrics.')
    ...
    parser.add_argument(
        '--output_dir',
        default='ML/autoencoder/experiments/LegacyCondFlow/results',
        help='Output directory for metrics.',
    )
    parser.add_argument(
        '--output_name',
        default='post_training_wflow_comparison_metrics.json',
    ...
    baseline, full_cov, flow, wflow = load_models(device=device, num_stations=len(station_list))

    metric_keys = ['ssim', 'lsd', 'sc', 's_corr', 'arias_err', 'env_corr', 'dtw', 'xcorr']
    results = {m: {k: [] for k in metric_keys} for m in ['Baseline', 'FullCov', 'Flow', 'WFlow']}
