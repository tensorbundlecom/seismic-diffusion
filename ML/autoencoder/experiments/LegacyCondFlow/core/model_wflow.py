...
class ConditionalAffineCouplingLayerW(nn.Module):
    """RealNVP affine coupling conditioned on legacy `w` condition vectors."""
...
class NormalizingFlowW(nn.Module):
    """Stack of legacy-`w` conditioned affine coupling layers."""
...
class WConditionedDecoder(nn.Module):
    """Decoder conditioned only on legacy `w` condition vectors (no direct raw condition path)."""
...
class WFlowCVAE(nn.Module):
    """CVAE + RealNVP where conditioning is mapped into legacy `w` and used everywhere."""
