...
from ML.autoencoder.experiments.LegacyCondFlow.core.loss_utils import wflow_cvae_loss_function


def train_wflow_external():
    ...
    exp_dir = Path('ML/autoencoder/experiments/LegacyCondFlow')
    checkpoint_dir = exp_dir / 'checkpoints'
    checkpoint_dir.mkdir(parents=True, exist_ok=True)

    print('[INFO] Starting LegacyCondFlow training')
    print(f'[INFO] Device={device} latent_dim={latent_dim} w_dim={w_dim} flow_layers={flow_layers}')
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            loss, recon_l, kl_l = wflow_cvae_loss_function(recon, specs, mu, logvar, zk, log_det, beta=beta)
            loss.backward()
            optimizer.step()
        ...
        torch.save(latest_payload, checkpoint_dir / 'wflow_external_latest.pt')

        if (epoch + 1) % 10 == 0:
            torch.save(latest_payload, checkpoint_dir / f'wflow_external_epoch_{epoch+1:03d}.pt')

        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            torch.save(latest_payload, checkpoint_dir / 'wflow_external_best.pt')
            print(f'[INFO] New best model saved: val_loss={best_val_loss:.2f}')

    print('[INFO] LegacyCondFlow training completed.')


if __name__ == '__main__':
    train_wflow_external()
