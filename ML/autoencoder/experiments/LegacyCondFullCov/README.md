# LegacyCondFullCov CVAE Experiment
...
Bu deney, FullCovariance CVAE'yi legacy `w` condition-embedding ile yeniden kurar.
...
## Klasor Yapisi
- `core/model_wfullcov.py`: legacy-`w` conditioned full covariance model
...
```bash
setsid bash -lc '/home/gms/.pyenv/shims/python -u ML/autoencoder/experiments/LegacyCondFullCov/training/train_wfullcov_external.py > ML/autoencoder/experiments/LegacyCondFullCov/logs/train/train_wfullcov_external_$(date +%Y%m%d_%H%M%S).log 2>&1 < /dev/null & echo $! > ML/autoencoder/experiments/LegacyCondFullCov/logs/train/train_wfullcov_external.pid; disown' >/dev/null 2>&1
```
