...
def main():
    parser = argparse.ArgumentParser(description="Visualize WFullCov post-training OOD (specs + waveforms).")
    ...
    parser.add_argument(
        "--output_dir",
        default="ML/autoencoder/experiments/LegacyCondFullCov/visualizations/post_training_ood_evaluation",
    ...
    wfullcov_chk = 'ML/autoencoder/experiments/LegacyCondFullCov/checkpoints/wfullcov_external_best.pt'
    ...
    wfc_state = torch.load(wfullcov_chk, map_location=device)
    cfg = wfc_state.get('config', {})
    ...
    wfullcov.load_state_dict(wfc_state['model_state_dict'])
    wfullcov.eval()
    ...
    with torch.no_grad():
        for i in range(len(dataset)):
            ...
            wav_wfc = reconstruct_signal(wfc_spec, mag_min, mag_max, fs=target_fs)

            min_len = min(len(gt_wav), len(wav_base), len(wav_fc), len(wav_flow), len(wav_wfc))
            ...
            m_wfc = calculate_metrics(gt_wav[:min_len], wav_wfc[:min_len], orig_spec, wfc_spec, fs=target_fs)
            ...
            axes[1, 1].imshow(wfc_spec, aspect='auto', origin='lower', cmap='viridis')
            axes[1, 1].set_title(f'WFullCov (SSIM: {m_wfc["ssim"]:.3f})')
            axes[1, 2].axis('off')
            plt.tight_layout()
            ...
            fwc, faswc = get_fas(wav_wfc[:min_len], fs=target_fs)
            ...
            plt.loglog(fwc, faswc, 'm', alpha=0.8, label=f'WFullCov (LSD: {m_wfc["lsd"]:.2f})', linewidth=0.9)
            ...
            axes[4].plot(time, wav_wfc[:min_len], 'm-', linewidth=0.8, label=f'WFullCov (XCorr: {m_wfc["xcorr"]:.3f})')
