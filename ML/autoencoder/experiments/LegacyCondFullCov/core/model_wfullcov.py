...
class MappingNetwork(nn.Module):
    """Maps normalized physical parameters + station embedding into a condition embedding."""
...
class WConditionedDecoder(nn.Module):
    """Decoder conditioned only on the legacy `w` condition embedding."""
...
class WFullCovCVAE(nn.Module):
    """Full covariance CVAE with legacy `w` condition embedding everywhere (no flow)."""
    ...
    def _initialize_fc_layers(self, flattened_dim, device):
        in_dim = flattened_dim + self.w_dim
    ...
    def forward(self, x, magnitude, location, station_idx):
        ...
        chol_flat = self.fc_chol(posterior_input)

        L = torch.zeros(batch_size, self.latent_dim, self.latent_dim, device=x.device)
        tril_idx = torch.tril_indices(row=self.latent_dim, col=self.latent_dim, offset=0)
