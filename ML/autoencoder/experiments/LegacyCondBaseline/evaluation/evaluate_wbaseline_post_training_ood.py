...
def calculate_metrics(target_wav, pred_wav, target_spec, pred_spec, fs=100.0):
    ...
    a_target = arias(target_wav)
    a_pred = arias(pred_wav)
...
def load_models(device, num_stations):
    ...
    for p in [baseline_chk, full_cov_chk, flow_chk, wbaseline_chk]:
    ...
    return baseline, full_cov, flow, wbaseline


def main():
    parser = argparse.ArgumentParser(description='Evaluate WBaseline post-training OOD metrics.')
    ...
    parser.add_argument(
        '--output_dir',
        default='ML/autoencoder/experiments/LegacyCondBaseline/results',
        help='Output directory for metrics.',
    )
    parser.add_argument(
        '--output_name',
        default='post_training_wbaseline_comparison_metrics.json',
    ...
    baseline, full_cov, flow, wbaseline = load_models(device=device, num_stations=len(station_list))

    metric_keys = ['ssim', 'lsd', 'sc', 's_corr', 'arias_err', 'env_corr', 'dtw', 'xcorr']
    results = {m: {k: [] for k in metric_keys} for m in ['Baseline', 'FullCov', 'Flow', 'WBaseline']}
    ...
    out_file = os.path.join(output_dir, args.output_name)
