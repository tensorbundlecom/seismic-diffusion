...
def main():
    parser = argparse.ArgumentParser(description="Visualize WBaseline post-training OOD (specs + waveforms).")
    ...
    parser.add_argument(
        "--station_subset_file",
        default=None,
        help="Optional station subset JSON to filter visualization (keeps full mapping for models).",
    )
    parser.add_argument(
        "--output_dir",
        default="ML/autoencoder/experiments/LegacyCondBaseline/visualizations/post_training_ood_evaluation",
    ...
    wbaseline_chk = 'ML/autoencoder/experiments/LegacyCondBaseline/checkpoints/wbaseline_external_best.pt'
    ...
    wb_state = torch.load(wbaseline_chk, map_location=device)
    cfg = wb_state.get('config', {})
    wbaseline = WBaselineCVAE(
        in_channels=3,
        latent_dim=cfg.get('latent_dim', 128),
        num_stations=cfg.get('num_stations', num_stations),
        w_dim=cfg.get('w_dim', 64),
    ).to(device)
    wbaseline.load_state_dict(wb_state['model_state_dict'])
    wbaseline.eval()
    ...
    with torch.no_grad():
        for i in range(len(dataset)):
            ...
            wav_wb = reconstruct_signal(wb_spec, mag_min, mag_max, fs=target_fs)

            min_len = min(len(gt_wav), len(wav_base), len(wav_fc), len(wav_flow), len(wav_wb))
            m_base = calculate_metrics(gt_wav[:min_len], wav_base[:min_len], orig_spec, base_spec, fs=target_fs)
            m_fc = calculate_metrics(gt_wav[:min_len], wav_fc[:min_len], orig_spec, fc_spec, fs=target_fs)
            m_flow = calculate_metrics(gt_wav[:min_len], wav_flow[:min_len], orig_spec, flow_spec, fs=target_fs)
            m_wb = calculate_metrics(gt_wav[:min_len], wav_wb[:min_len], orig_spec, wb_spec, fs=target_fs)
            ...
            axes[0, 2].set_title(f'FullCov (SSIM: {m_fc["ssim"]:.3f})')
            axes[1, 0].imshow(flow_spec, aspect='auto', origin='lower', cmap='viridis')
            axes[1, 0].set_title(f'Flow (SSIM: {m_flow["ssim"]:.3f})')
            axes[1, 1].imshow(wb_spec, aspect='auto', origin='lower', cmap='viridis')
            axes[1, 1].set_title(f'WBaseline (SSIM: {m_wb["ssim"]:.3f})')
            axes[1, 2].axis('off')
            plt.tight_layout()
            ...
            fwb, faswb = get_fas(wav_wb[:min_len], fs=target_fs)
            ...
            plt.loglog(fwb, faswb, 'm', alpha=0.8, label=f'WBaseline (LSD: {m_wb["lsd"]:.2f})', linewidth=0.9)
            ...
            axes[4].plot(time, wav_wb[:min_len], 'm-', linewidth=0.8, label=f'WBaseline (XCorr: {m_wb["xcorr"]:.3f})')
