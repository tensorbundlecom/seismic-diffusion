...
from ML.autoencoder.experiments.LegacyCondBaseline.core.loss_utils import wbaseline_cvae_loss_function
from ML.autoencoder.experiments.LegacyCondBaseline.core.model_wbaseline import WBaselineCVAE


def train_wbaseline_external():
    ...
    exp_dir = Path('ML/autoencoder/experiments/LegacyCondBaseline')
    checkpoint_dir = exp_dir / 'checkpoints'
    checkpoint_dir.mkdir(parents=True, exist_ok=True)

    print('[INFO] Starting LegacyCondBaseline training')
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            loss, recon_l, kl_l = wbaseline_cvae_loss_function(recon, specs, mu, logvar, beta=beta)
            loss.backward()
            optimizer.step()
        ...
        torch.save(payload, checkpoint_dir / 'wbaseline_external_latest.pt')

        if (epoch + 1) % 10 == 0:
            torch.save(payload, checkpoint_dir / f'wbaseline_external_epoch_{epoch+1:03d}.pt')

        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            torch.save(payload, checkpoint_dir / 'wbaseline_external_best.pt')
            print(f'[INFO] New best model saved: val_loss={best_val_loss:.2f}')

    print('[INFO] LegacyCondBaseline training completed.')


if __name__ == '__main__':
    train_wbaseline_external()
