...
def download_single_station(client, network, sta, starttime, endtime, event_id, output_dir, channel_filter="HH?"):
    """Download HH channels for a single station/event."""
    try:
...
def download_post_training_ood_custom(
    catalog_path,
    output_dir,
    network="KO",
    duration_sec=60,
    ...
    """Download HH channel waveforms for custom post-training OOD events."""
    clients = []
    try:
        clients.append(("KOERI", Client("KOERI", timeout=60)))
    except Exception as e:
        print(f"[WARN] KOERI client init failed: {e}")
    try:
        clients.append(("IRIS", Client("IRIS", timeout=60)))
    except Exception as e:
    ...
    print(f"Downloading HH waveforms for {len(df)} post-training OOD events...")
    print("Stations: ADVT, ARMT, KCTX, YLV, GEML, GELI")

    target_stations = ["ADVT", "ARMT", "KCTX", "YLV", "GEML", "GELI"]
...
if __name__ == "__main__":
    ...
    out_dir = "data/ood_waveforms/post_training_custom"
    download_post_training_ood_custom(catalog, out_dir)
