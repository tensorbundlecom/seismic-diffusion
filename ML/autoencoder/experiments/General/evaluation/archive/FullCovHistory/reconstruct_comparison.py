...
def reconstruct_signal(magnitude_spec, nperseg=256, noverlap=192, n_iter=64, log_scale=True, normalize=True):
    """
    Reconstruct signal from magnitude spectrogram using Griffin-Lim.
    magnitude_spec: (freq_bins, time_bins) - [0, 1] log-scaled or similar
    """
    spec = magnitude_spec.copy()
    
    # Actually, we don't have the original min/max for inversion if it was normalized per-sample.
    # But we can at least undo the log-scale which is the main distortion.
    if log_scale:
        # If it was log(1+x), then inverse is exp(x)-1
        spec = np.expm1(spec)
