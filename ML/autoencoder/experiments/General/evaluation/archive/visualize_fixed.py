...
def main():
    ...
    output_dir = "ML/autoencoder/experiments/General/visualizations/final_external_fixed"
    ...
    with torch.no_grad():
        for i in range(len(dataset)):
            try:
                ...
                base_station_in = station_idx.unsqueeze(0).to(device)
                ...
                # Create time vectors for reconstructed waveforms (assume 100 Hz sample rate from STFT params)
                recon_sr = 100.0  # Approximate sample rate from STFT params
                base_time = np.arange(len(base_wav)) / recon_sr
                fc_time = np.arange(len(fc_wav)) / recon_sr
                flow_time = np.arange(len(flow_wav)) / recon_sr
                
                # Debug print
                print(f"{event_name}: filt_len={len(filt_data)}, base_len={len(base_wav)}, fc_len={len(fc_wav)}, flow_len={len(flow_wav)}")
                print(f"  filt_range=[{filt_data.min():.2f}, {filt_data.max():.2f}], base_range=[{base_wav.min():.2f}, {base_wav.max():.2f}]")
                print(f"  fc_range=[{fc_wav.min():.2f}, {fc_wav.max():.2f}], flow_range=[{flow_wav.min():.2f}, {flow_wav.max():.2f}]")
                
                # Plot - each with its own time axis
                fig, axes = plt.subplots(4, 1, figsize=(14, 12))
                fig.suptitle(f"External Model Comparison: {event_name}", fontsize=16)
                
                axes[0].plot(filt_time, filt_data, 'black', lw=0.5)
                axes[0].set_title(f"Original Filtered (len={len(filt_data)})")
                axes[0].set_ylabel("Amplitude")
                
                axes[1].plot(base_time, base_wav, 'blue', lw=0.5)
                axes[1].set_title(f"Baseline 125st (len={len(base_wav)})")
                axes[1].set_ylabel("Amplitude")

                axes[2].plot(fc_time, fc_wav, 'red', lw=0.5)
                axes[2].set_title(f"FullCov External 46st (len={len(fc_wav)})")
                axes[2].set_ylabel("Amplitude")

                axes[3].plot(flow_time, flow_wav, 'purple', lw=0.5)
                axes[3].set_title(f"Flow External 46st (len={len(flow_wav)})")
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(output_dir, f"fixed_{event_name}.png"), dpi=150)
                plt.close()
                print(f"Saved fixed_{event_name}.png")
