...
def griffin_lim(mag_stft, n_iter=32, n_fft=256, hop_length=64):
    """Simple Griffin-Lim for phase recovery."""
    noverlap = n_fft - hop_length
    phase = np.random.randn(*mag_stft.shape)
    stft = mag_stft * np.exp(1j * phase)
    ...
    for _ in range(n_iter):
        ...
        new_stft = new_stft[:, :mag_stft.shape[1]]
