#!/usr/bin/env python3
"""
Train a single NonDiagonalRigid V1 run with frozen splits/stats.

This script does not modify legacy experiment folders.
"""
...
def build_model(model_family: str, backbone: str, latent_dim: int, condition_dim: int, num_stations: int, device: torch.device):
    numeric_condition_dim = 5
    if model_family == "baseline_diag" and backbone == "large":
    ...
    elif model_family == "baseline_diag" and backbone == "small":
    ...
    elif model_family == "fullcov" and backbone == "large":
    ...
    elif model_family == "fullcov" and backbone == "small":
        ...
        raise ValueError(f"Unsupported pair: model_family={model_family}, backbone={backbone}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train one NonDiagonalRigid model with frozen data protocol.")
    parser.add_argument("--model_family", choices=["baseline_diag", "fullcov"], required=True)
    parser.add_argument("--backbone", choices=["large", "small"], required=True)
    ...
    run_name = args.run_name or f"rigid_{args.model_family}_{args.backbone}_ld{args.latent_dim}_s{args.seed}_{ts}"
    ...
    # Dataset (ID only), frozen station set, frozen normalization
    ...
    # Backward-compatible fallback if loader expects stats under `features`
    ...
    logger.info(
        f"RUN={run_name} model={args.model_family}/{args.backbone} ld={args.latent_dim} "
