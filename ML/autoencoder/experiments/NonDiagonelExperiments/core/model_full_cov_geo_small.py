...
from ML.autoencoder.experiments.NonDiagonel.core.model_baseline_geo_small import (
    SmallGeoCVAEDecoder,
    SmallGeoCVAEEncoder,
)


class SmallGeoFullCovEncoder(nn.Module):
    """
    Capacity-reduced full-covariance encoder producing mean and Cholesky factor L.
    """
...
class SmallGeoFullCovCVAE(nn.Module):
    def __init__(
        self,
        in_channels: int = 3,
        latent_dim: int = 128,
        ...
        super().__init__()
        self.encoder = SmallGeoFullCovEncoder(
            in_channels=in_channels,
            latent_dim=latent_dim,
            num_stations=num_stations,
            condition_dim=condition_dim,
