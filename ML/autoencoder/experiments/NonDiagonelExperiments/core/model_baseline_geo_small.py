...
class SmallGeoCVAEEncoder(nn.Module):
    """
    Capacity-reduced conditional encoder (small backbone) with geometry features + station embedding.
    """
...
class SmallGeoCVAEDecoder(nn.Module):
    def __init__(
        self,
        out_channels: int = 3,
        latent_dim: int = 128,
        ...
        self.spatial_dim = 16
        self.projection_dim = 192 * self.spatial_dim * self.spatial_dim
        ...
        self.decoder = nn.Sequential(
            ...
            nn.BatchNorm2d(24),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(24, out_channels, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.Sigmoid(),
        )
    ...
    def forward(self, z: torch.Tensor, cond_numeric: torch.Tensor, station_idx: torch.Tensor):
        ...
        h = h.view(batch_size, 192, self.spatial_dim, self.spatial_dim)
        return self.decoder(h)


class SmallGeoConditionalVariationalAutoencoder(nn.Module):
    def __init__(
        self,
        in_channels: int = 3,
        latent_dim: int = 128,
        ...
        super().__init__()
        self.encoder = SmallGeoCVAEEncoder(
        ...
        self.decoder = SmallGeoCVAEDecoder(
