...
from ML.autoencoder.experiments.NonDiagonel.core.loss_utils import beta_cvae_loss
...
def build_checkpoint_payload(model, optimizer, epoch, val_loss, config, stats_path, save_optimizer):
    payload = {
        "epoch": epoch,
...
def safe_torch_save(payload, target_path: Path, logger: logging.Logger) -> bool:
    ...
    try:
        ...
        logger.error(f"[CKPT-ERROR] path={target_path} error={e}")
        try:
            if tmp_path.exists():
                tmp_path.unlink()
        except OSError:
...
def main():
    parser = argparse.ArgumentParser(description="Train small-backbone geometry-conditioned baseline CVAE on external HH dataset.")
    ...
    parser.add_argument("--save_optimizer", type=int, default=0, help="Save optimizer state in checkpoints (1/0).")
    parser.add_argument("--save_epoch_every", type=int, default=0, help="Save periodic epoch checkpoints every N epochs (0 disables).")
    ...
    run_name = args.run_name or f"baseline_geo_small_external_{ts}"
    ...
    # Fit normalization stats on train split only, unless provided externally.
    ...
    config = {
        "run_name": run_name,
        "model": "SmallGeoConditionalVariationalAutoencoder",
        "backbone_scale": "small_24_48_96_192",
        "device": str(device),
        "epochs": args.epochs,
        "batch_size": args.batch_size,
        "num_workers": args.num_workers,
    ...
    try:
        ...
        logger.error(f"[CONFIG-ERROR] path={cfg_path} error={e} (training will continue)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        payload = build_checkpoint_payload(
            ...
            save_optimizer=(args.save_optimizer == 1),
        )

        latest_path = ckpt_dir / f"{run_name}_latest.pt"
        safe_torch_save(payload, latest_path, logger)

        if row["val_loss"] < best_val:
            best_val = row["val_loss"]
            best_path = ckpt_dir / f"{run_name}_best.pt"
            safe_torch_save(payload, best_path, logger)
        ...
        if args.save_epoch_every > 0 and epoch % args.save_epoch_every == 0:
            ...
            safe_torch_save(payload, ep_path, logger)
